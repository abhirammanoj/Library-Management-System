<?php
//this represents student sign up form
include("setting.php");
$name=$_POST['name'];
$email=$_POST['email'];
$sem=$_POST['sem'];
...
$pas=sha1($_POST['pass']);
$no=$_POST['contact'];
if($name==NULL || $email==NULL || $sem==NULL || $branch==NULL || $sid==NULL || $_POST['pass']==NULL || $no==NULL)
{
	//
}
else
{
	$sql=mysqli_query($set,"INSERT INTO students(sid,name,branch,sem,password,email,contactNO) VALUES('$sid','$name','$branch','$sem','$pas','$email','$no')");
	if($sql)
	{
